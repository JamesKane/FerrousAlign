...
fn test_reproduce_batch_truncation_bug() {
    ...
    assert_eq!(results.len(), num_jobs, "Batch truncation bug: expected {} results, got {}", num_jobs, results.len());
}
...
fn test_reproduce_long_read_stride_bug() {
    ...
    // Check count (Bug 2)
    assert_eq!(results.len(), num_jobs, "Batch truncation bug (i16 path): expected {} results, got {}", num_jobs, results.len());
    ...
    assert!(score_0 > 100, "Stride bug detected! Score {} is too low for perfect match.", score_0);
}
