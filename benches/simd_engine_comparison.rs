...
fn bench_alignment_complexity(c: &mut Criterion) {
    ...
    for mutation_rate in [0.0, 0.05, 0.10, 0.20].iter() {
        let alignments: Vec<(Vec<u8>, Vec<u8>)> = (0..num_alignments).map(|i| {
            let query = generate_random_sequence(seq_len, 42 + i as u64);
            let target = generate_sequence_with_mutations(&query, *mutation_rate, 123 + i as u64);
            (query, target)
        }).collect();
...
criterion_group!(benches, bench_simd_engine_comparison, bench_sequence_lengths, bench_alignment_complexity);
