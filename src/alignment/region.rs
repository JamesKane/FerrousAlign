...
pub fn generate_cigar_from_region(
    ...
    log::debug!(
        "CIGAR_INPUT: query_len={} query_first_10={:?} rb={} is_rev={}",
        query.len(),
        &query[..query.len().min(10)],
        region.rb,
        region.rb >= bwa_idx.bns.packed_sequence_length
    );
    ...
    // NOTE: qb/qe positions are ALWAYS relative to the original query, regardless of strand.
    // The FM-index bidirectional search determines strand based on position:
    // - Positions in [0, l_pac): forward strand alignments
    // - Positions in [l_pac, 2*l_pac): reverse strand alignments
    // SMEMs are generated by searching the original query, so query coordinates are
    // always in the original orientation.
    ...
    if qb >= qe || qe > query.len() {
        log::debug!(
            "Invalid query bounds: qb={}, qe={}, query_len={} (is_rev={})",
            qb,
            qe,
            query.len(),
            is_reverse_strand
    ...
    log::debug!(
        "QUERY_SLICE: is_rev={} qb={} qe={} slice_len={} slice_first_10={:?}",
        is_reverse_strand,
        qb,
        qe,
        query_segment.len(),
        &query_segment[..query_segment.len().min(10)]
    );
    ...
    // Handle reverse strand alignment
    // For reverse strand (is_reverse_strand=true):
    // - The FM-index found a match where rev_comp(query) matches the forward reference
    // - get_reference_segment already returns rev_comp(forward_reference) for reverse strand
    // - So rev_comp(forward_reference) should match the ORIGINAL query directly
    // - We should NOT reverse-complement the query again
    //
    // The alignment is: query[qb:qe] aligns to rev_comp(forward_ref)
    let (query_for_sw, rseq_for_sw) = if is_reverse_strand {
        log::debug!(
            "REV_STRAND_SW: qb={} qe={} query_first_10={:?} ref_first_10={:?}",
            qb,
            qe,
            &query_segment[..query_segment.len().min(10)],
            &rseq[..rseq.len().min(10)]
        );
        // For reverse strand: query_segment aligns directly to rseq (which is already rev-comp'd)
...
mod tests {
    ...
    // ========================================================================
    //
    // The bounds check at lines 1081-1096 requires accurate CIGAR reference
    // length calculation. These tests verify the logic matches SAM specification.
    // ========================================================================

    /// Calculate reference length from CIGAR (M, D, =, X consume reference)
    ...
    fn test_cigar_ref_len_with_eq_x_ops() {
        // 40=10X50= uses =/X instead of M - same reference consumption
    ...
    // CIGAR_MAPS_OFF_REFERENCE errors (lines 1081-1096).
    // ========================================================================
    ...
    // ========================================================================
    // INFER_BAND_WIDTH TESTS
    // ========================================================================
    //
    // Tests for the band width inference function (lines 1201-1240).
    // ========================================================================
    ...
    fn test_infer_band_width_different_lengths() {
        // When query and reference have different lengths, band width
        // must be at least the difference
        let w = infer_band_width(
            100, // l_query
            90,  // l_ref (10bp shorter)
            80,  // score
            1,   // match_score
            6,   // o_del
            1,   // e_del
        ...
        // Should be at least |100 - 90| = 10
        assert!(w >= 10);
    }

    #[test]
    fn test_infer_band_width_capped_at_4x_cmd() {
        // Band width should not exceed 4x command-line width
        let w = infer_band_width(
            1000, // l_query (very long)
            500,  // l_ref (big difference)
            100,  // score (low)
            1,    // match_score
            6,    // o_del
            1,    // e_del
