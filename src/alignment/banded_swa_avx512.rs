...
pub unsafe fn simd_banded_swa_batch64(
    ...
    const SIMD_WIDTH: usize = 64; // <Engine as crate::compute::simd_abstraction::SimdEngine>::WIDTH_8 (64-way parallelism)
...
pub unsafe fn simd_banded_swa_batch32_int16(
    ...
    for i in 0..max_tlen as usize {
        ...
        let t_vec = _mm512_loadu_si512(target_soa_16.as_ptr().add(i * SIMD_WIDTH) as *const __m512i);
        ...
        for j in global_beg..global_end.min(MAX_SEQ_LEN) {
            ...
            // and E from e_matrix
            let h00_vec = _mm512_loadu_si512(h_matrix.as_ptr().add(j * SIMD_WIDTH) as *const __m512i);
            let e_vec = _mm512_loadu_si512(e_matrix.as_ptr().add(j * SIMD_WIDTH) as *const __m512i);
            ...
            let q_vec = _mm512_loadu_si512(query_soa_16.as_ptr().add(j * SIMD_WIDTH) as *const __m512i);
            ...
            let score_vec = _mm512_mask_blend_epi16(cmp_eq, mismatch_score_vec, match_score_vec);
            ...
            _mm512_storeu_si512(h_matrix.as_mut_ptr().add(j * SIMD_WIDTH) as *mut __m512i, h1_vec);
            ...
            _mm512_storeu_si512(e_matrix.as_mut_ptr().add(j * SIMD_WIDTH) as *mut __m512i, new_e_vec);
...
mod tests {
    ...
    fn test_simd_banded_swa_batch32_int16_basic() {
        ...
        assert!(results[0].score > 0, "Score {} should be > 0", results[0].score);
    }
