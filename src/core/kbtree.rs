    ...
    V: Copy + Default,
{
    ...
    #[inline]
    unsafe fn search_node(&self, node: *mut u8, key: &K) -> (i32, bool) {
        ...
        while begin < end {
            let mid = (begin + end) / 2;
            match (*keys.offset(mid as isize)).0.cmp(key) {
        ...
        if begin == 0 {
            ...
            let idx = begin - 1;
            let found = (*keys.offset(idx as isize)).0.cmp(key) == Ordering::Equal;
            (idx, found)
        }
    }
    ...
    pub fn interval(&self, key: &K) -> (Option<&(K, V)>, Option<&(K, V)>) {
        if self.n_keys == 0 {
            return (None, None);
        }
        ...
        unsafe {
            ...
            while !x.is_null() {
                let header = self.header(x);
                let n = header.n();
                let keys = self.keys(x);

                let (i, found) = self.search_node(x, key);

                if found {
                    // Exact match
                    let entry = keys.offset(i as isize);
                    return (Some(&*entry), Some(&*entry));
                }

                // Update lower bound (largest key <= query)
                if i >= 0 {
                    lower = keys.offset(i as isize);
                }

                // Update upper bound (smallest key > query)
                let next_idx = i + 1;
                if next_idx < n as i32 {
                    upper = keys.offset(next_idx as isize);
                }
    ...
    pub fn get_lower_mut(&mut self, key: &K) -> Option<&mut (K, V)> {
        ...
        unsafe {
            ...
            while !x.is_null() {
                ...
                if found {
                    return Some(&mut *keys.offset(i as isize));
                }
                ...
                x = *self.children(x).offset((i + 1) as isize);
            }
    ...
    unsafe fn split_child(&mut self, x: *mut u8, i: usize, y: *mut u8) {
        let header_y = self.header(y);
        let y_internal = header_y.is_internal();

        // Allocate new node z
        let z = self.alloc_node(y_internal);

        let header_z = self.header(z);
        header_z.set_n(self.t - 1);

        // Copy upper half of y's keys to z
        let keys_y = self.keys(y);
        let keys_z = self.keys(z);
        ptr::copy_nonoverlapping(keys_y.add(self.t), keys_z, self.t - 1);

        // Copy upper half of y's children to z (if internal)
        if y_internal {
            let children_y = self.children(y);
            let children_z = self.children(z);
            ptr::copy_nonoverlapping(children_y.add(self.t), children_z, self.t);
        }
        ...
        let children_x = self.children(x);
        let header_x = self.header(x);
        let n_x = header_x.n();

        for j in (i + 1..=n_x).rev() {
            *children_x.add(j + 1) = *children_x.add(j);
        }
        *children_x.add(i + 1) = z;

        // Shift x's keys and insert median from y
        let keys_x = self.keys(x);
        for j in (i..n_x).rev() {
            *keys_x.add(j + 1) = *keys_x.add(j);
        }
        *keys_x.add(i) = *keys_y.add(self.t - 1);
    ...
    unsafe fn insert_nonfull(&mut self, x: *mut u8, key: K, value: V) {
        ...
        if !header.is_internal() {
            ...
            while i >= 0 && (*keys.offset(i as isize)).0 > key {
                *keys.offset((i + 1) as isize) = *keys.offset(i as isize);
                i -= 1;
            }
            *keys.offset((i + 1) as isize) = (key, value);
            header.inc_n();
        } else {
            ...
            let child_header = self.header(child);

            if child_header.n() == self.n {
                // Child is full, split it
                self.split_child(x, i as usize, child);
                if (*keys.offset(i as isize)).0 < key {
                    i += 1;
                }
            }

            self.insert_nonfull(*children.offset(i as isize), key, value);
        }
    }
    ...
    pub fn insert(&mut self, key: K, value: V) {
        ...
        unsafe {
            let root_header = self.header(self.root);
        ...
        F: FnMut(&K, &V),
    {
        ...
        if header.is_internal() {
            ...
            for i in 0..n {
                self.traverse_node(*children.add(i), f);
                let entry = &*keys.add(i);
                f(&entry.0, &entry.1);
            }
            self.traverse_node(*children.add(n), f);
        } else {
            for i in 0..n {
                let entry = &*keys.add(i);
                f(&entry.0, &entry.1);
...
mod tests {
    ...
    #[test]
    fn test_empty_tree() {
        let tree: KBTree<u64, i32> = KBTree::new();
