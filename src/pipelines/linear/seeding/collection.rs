...
/// Per-read seeding result for parallel processing.
struct PerReadSeedResult {
    /// Seeds for this read
    seeds: Vec<Seed>,
    /// Encoded query sequence
    encoded_query: Vec<u8>,
    /// Encoded reverse complement query
    encoded_query_rc: Vec<u8>,
}

/// Process a single read and generate seeds.
///
/// This is the parallelizable unit of work for seeding.
fn find_seeds_single_read(
    bwa_idx: &BwaIndex,
    query_name: &str,
    query_seq: &[u8],
    opt: &MemOpt,
    ...
    PerReadSeedResult {
        seeds: current_read_seeds,
        encoded_query,
        encoded_query_rc,
    }
}

/// Find seeds for a batch of reads using per-read parallelism.
///
/// This is the main entry point for seeding. It uses rayon to process each
/// read in parallel, providing fine-grained parallelism that avoids the
/// straggler effect when some reads are pathologically slow to seed.
///
/// # Algorithm
...
pub fn find_seeds_batch(
    ...
    if num_reads == 0 {
        return (
            SoASeedBatch::new(),
            SoAEncodedQueryBatch::new(),
            SoAEncodedQueryBatch::new(),
        );
    }

    // Process each read in parallel using rayon
    let per_read_results: Vec<PerReadSeedResult> = (0..num_reads)
        .into_par_iter()
        .map(|read_idx| {
            ...
            find_seeds_single_read(bwa_idx, query_name, query_seq, opt)
        })
        .collect();

    // Merge per-read results into SoA batches (sequential merge to maintain order)
    ...
    for (read_idx, result) in per_read_results.into_iter().enumerate() {
        let query_len = read_batch.read_boundaries[read_idx].1;
        ...
        soa_encoded_query_batch
            .encoded_seqs
            .extend_from_slice(&result.encoded_query);
        soa_encoded_query_batch
            .query_boundaries
        ...
        soa_encoded_query_rc_batch
            .encoded_seqs
            .extend_from_slice(&result.encoded_query_rc);
        soa_encoded_query_rc_batch
            .query_boundaries
            .push((current_encoded_query_rc_start, query_len));

        // Store seeds in SoA batch
